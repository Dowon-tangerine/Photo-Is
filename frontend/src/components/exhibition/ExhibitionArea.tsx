				 ...
				 if(index !== -1 && title == ''){
					 exhibitionPhotos.content.forEach((paint: any) => {
						 if(paint.number === index) {
							...
							setMetaData(paint.photoMetadata);
							setShowDetail(true);
						 }
					 })
				 }
    ...
    <div className={styles.main_container}>
      ...
      <div className={styles.tutorial}>
        <div className={styles.description_exit}>
          ...
          <h1 style={{fontFamily: "부크크고딕bold", fontSize:'25px'}}>나가고 싶을 때 클릭하세요</h1>
        </div>
        <div className={styles.description_move}>
          <img src='imgs/keyboard.png' alt='arrow'/>
          <h1 style={{fontFamily: "부크크고딕bold", fontSize:'25px', marginLeft:'-20px', marginBottom:'20px'}}>방향키 혹은 W,A,S,D로 이동하세요</h1>
        </div>
        <div className={styles.description_camera}>
          <img src='imgs/directional_arrow.png' alt='arrow'/>
          <h1 style={{fontFamily: "부크크고딕bold", width:'990px', marginLeft:'-190px', fontSize:'25px'}}>클릭 & ESC키를 눌러 모드를 전환하세요!</h1>
        </div>
        <div className={styles.exit_tutorial} onClick={handleCloseClick} style={{fontSize:'30px',padding:'20px', fontFamily: "부크크고딕bold"}}>튜토리얼 닫기</div>
      </div>
      }
      <div className={styles.btn_exit} onClick={()=>{navigate(-1);}}>Exit</div>
      {
        isLoading && 
          <div className={styles.loading}>
            <div>
              <Circle style={{width: '200px'}} percent={loadingPercent} strokeWidth={10} strokeColor='white'/>
              <h1 style={{color: 'white', width: '200px', margin:'20px auto', fontFamily: "부크크명조bold"}}>{loadingPercent}%<br></br>Loading...</h1>
            </div>
          </div>
      }
